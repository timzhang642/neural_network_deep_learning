...
function [weights, biases] = Expanded_NN(input,target,split,nodeLayers,numEpochs,batchSize,eta,trans,cost,lambda,mu)
    ...
    % initiate lists for performance plotting
    epoch_list = []; 
    train_cost_list = [];
    valid_cost_list = [];
    test_cost_list = [];
    ...
    % placeholder for velocities of all weights and biases, will be used for momentum parameter updates
    ...
    for epoch=1:numEpochs
        ...
        for index = 1:batchSize:size(cat_data,2)
            ...
            % the first elem of a is input nodes, which are mini-batch training examples
            a{1} = mini_train; 

            % feed-forward
            for conn=1:length(nodeLayers)-1 % for each connections between layers
                z{conn} = weights{conn}*a{conn} + biases{conn}; % weighted sum of nodes from previous layer and weights
            ...
            % back-propagation  
            % gradient of loss (delta) w.r.t. activation function in layers except the first layer.
            % If we have 3-layered network, then we compute delta for last two layers
            ...
            % compute gradient of loss w.r.t. parameters, including L2 regularization
        ...
        % get performance on training, validation and test sets
    ...
    % plot prediction cost for training, validation and testing sets
    figure,
